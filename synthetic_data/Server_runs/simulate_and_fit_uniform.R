...
set.seed(12345)
num_rep <- 3
t_meas_all <- seq(1/360, 1, length.out = 500)
...
downsample_set <- c(1,2,5,10,20,50)
for (i in 1:length(downsample_set)) {
  ...
  fit <- sampling(sm, data=data, iter=2000, seed=1234, control = list(adapt_delta=0.95));
  saveRDS(c(data=data,fit=fit), file=paste("FITS_unif/fit_",i, ".rds",sep=""))
}
